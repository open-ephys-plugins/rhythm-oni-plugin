...
BoardType RHD2000Thread::boardType = ACQUISITION_BOARD; // initialize the static member
...
{
    return new RHD2000Thread(sn, boardType);
}

RHD2000Thread::RHD2000Thread(SourceNode* sn, BoardType boardType_) : DataThread(sn),
    ...
    newScan(true), ledsEnabled(true), eventListener(this)
{
...
void RHD2000Thread::setTtlOutputState(int channel, bool state)
{
...
EventListener::EventListener(RHD2000Thread* board_) : Thread("Event Listener Thread"), board(board_)
{
...
void EventListener::run() 
{
    while (!threadShouldExit())
    {
        ...
        if (!connected)
        {
            connectedSocket = baseSocket.waitForNextConnection();
            connected = true;
        }
            
        int count = connectedSocket->read(buffer, 3, true);

        if (count > -1)
        {
            if (buffer[0] == 127 && buffer[1] < 8 && buffer[2] < 2)
            {
                board->setTtlOutputState((int)buffer[1], (bool)buffer[2]);
            }
        }
    }
